...
hops = "../dataset/hops.csv"
dataset = pd.read_csv(hops, sep=';')
'''print(dataset.head())
...
# elimino le righe con valori nulli nella colonna name
dataset = dataset.dropna(subset=['name'])

# elimino colonne con elevato numero di valori nulli
columns_to_drop = ['temperature', 'utilization', 'hop_temp']
...
for col in numeric_columns:
    ...
    plt.plot(values, label=col)  # grafico a linea dei valori
...
dataset.to_csv("../dataset_cleaned/hops_dataset_cleaned.csv", index=False)

'''range_valori = dataset['alpha'].agg(['min', 'max'])
print(range_valori)'''